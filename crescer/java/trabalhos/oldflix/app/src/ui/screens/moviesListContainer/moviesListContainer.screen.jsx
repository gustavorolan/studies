...
export const MovieListContainer = () => {
  ...
  async function getMoviesApi() {
    try {
      ...
      const moviesToShow = data.filter(
        (movie) => movie.itAvailable !== availableMovies
      );
      setMoviesList(data);
  ...
  React.useEffect(() => {
    getMoviesApi();
  }, [setMoviesList,setShowList]);

  const handleClickChangeListNotAvailable = () => {
    const movieListFiltered = moviesList.filter(
      (movie) => movie.itAvailable !== true
    );
  ...
  return (
    <div className="mainContainer">
      <ButtonToSelect
        handleClickChangeListNotAvailable={handleClickChangeListNotAvailable}
        handleClickChangeListAvailable={handleClickChangeListAvailable}
        handleClickToCreate={handleClickToCreate}
      />
      <div className="listContainer">
        {showList.map(
          ({ id, title, desc, category, itAvailable, imageUrl }) => (
            <MovieContainer
              imageUrl={imageUrl}
              handleRent={handleRent}
              handleDevolution={handleDevolution}
              itAvailable={itAvailable}
              key={id}
              id={id}
              name={title}
              description={desc}
