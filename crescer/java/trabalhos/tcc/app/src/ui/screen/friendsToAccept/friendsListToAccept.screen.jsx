...
export const FriendsToAccept = ({ setToast }) => {
  const [user, setUser] = useGlobalUser();
  const [friendsList, setFriendsList] = useState([]);
  const { getAllRequestsToBeFriends, acceptFriend } = useTccApi();

  const getUseApi = async () => {
    try {
      const response = await getAllRequestsToBeFriends(user.token);
  ...
  const handleAcceptFriend = async (idToRemove) => {
    try {
      const { data } = await acceptFriend(user.token, idToRemove);
      getUseApi();
        ...
        {friendsList.map(
          ({ userId, userName, nickName, email, profileImg }) => (
            <Friend
              ...
              handleRemoveFriend={handleAcceptFriend}
              buttonName={"Aceitar"}
            />
          )
        )}
